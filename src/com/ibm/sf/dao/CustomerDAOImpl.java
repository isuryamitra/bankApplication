...
private Logger daoLogger=Logger.getLogger(CustomerDAOImpl.class);
	...
	public Double getCurrentAmount(String uid) 
	{
		...
		try {
			...
			ResultSet resultSet=preparedStatement.executeQuery();
			if(resultSet.next()) {
				return resultSet.getDouble(columnIndex);//specify index of avaliable balance field
	...
	@Override
	public List getStatement(String uid) {	//get details from transfer table
	...
	@Override
	public Integer fundsTransfer(String name, String bid, Integer type, String ifsc, Double amount,String remarks) { 
		Connection connection=null;
		try {
			Context context= 
					(Context)new InitialContext().lookup("java:comp/env");
			...
			preparedStatement.setString(5,bid);
			//date
	...
	public Integer mobileTopUp(Long mobno, String operator, Double amount) {
		...
		try {
			...
			{
			ResultSet resultSet=preparedStatement.executeQuery();
			current_bal=withdraw(uid,amount);
			return 1;
			}else {
